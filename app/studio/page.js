  ...
  return (
    <main className="bg-slate-950 text-slate-50 flex flex-col overflow-hidden" style={{ height: "calc(100vh - 64px)" }}>
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all disabled:opacity-40 disabled:cursor-not-allowed ${
                saveStatus === "saved"
                  ...
                  : "bg-slate-800 border border-slate-700 hover:border-sky-500/60 hover:bg-sky-500/10 text-slate-300 hover:text-sky-300"
              }`}
            >
              {isSaving ? (
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {/* ── Desktop Sidebar ── */}
        <aside className="hidden lg:flex flex-col w-72 shrink-0 h-full bg-slate-900/95 border-r border-slate-800/50 backdrop-blur-md">
          {/* Tab Switcher */}
          <div className="p-3 shrink-0 border-b border-slate-800/50">
            ...
            {activeTab === "edit" ? (
              <div className="space-y-5">
                ...
                <div>
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <h3 className="text-sm font-bold text-slate-100">UV Mapping</h3>
                  </div>
                  ...
                  return (
                    <div>
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <h3 className="text-sm font-bold text-slate-100">Select Component</h3>
                      </div>
                ...
                <div>
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <h3 className="text-sm font-bold text-slate-100">Base Color</h3>
                  </div>
                  <div className="flex items-center gap-3 p-3 rounded-xl border border-slate-800 bg-slate-950/50">
                    <input
                      ...
                      className="h-12 w-12 rounded-xl border-2 border-slate-700 bg-slate-900 cursor-pointer hover:border-sky-500 transition-colors"
                    ...
                    <input
                      ...
                      value={baseColor}
                      onChange={(e) => setBaseColor(e.target.value)}
                      className="flex-1 rounded-lg border border-slate-700 bg-slate-900 px-3 py-2.5 text-sm font-mono text-slate-100 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-transparent transition-all"
                      placeholder="#ffffff"
                    />
                  </div>
                </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h2 className="text-sm font-bold text-slate-100">Select Model</h2>
                </div>
                  ...
                  {MODEL_OPTIONS.map((model) => {
                    ...
                    return (
                      <div key={model.id} className="space-y-2">
                        <button
                          onClick={() => { setSelectedModel(model.id); setSelectedVariation(null); }}
                          className={`w-full flex items-center gap-3 px-3 py-3 rounded-xl border-2 transition-all duration-200 group ${
                            selectedModel === model.id
                              ? "border-sky-500 bg-gradient-to-r from-sky-500/15 to-cyan-500/10 text-sky-100 shadow-lg shadow-sky-500/10"
                        ...
                        >
                          ...
                          <span className="font-semibold flex-1 text-left text-sm">{model.label}</span>
                          {hasModels && (
                            <span className="text-xs text-slate-500 bg-slate-800/60 px-1.5 py-0.5 rounded-md">
                              {categoryModels.length}
                            </span>
                          )}
                          <svg className={`w-4 h-4 transition-transform duration-200 ${selectedModel === model.id ? "rotate-90 text-sky-400" : "text-slate-600"}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                          </svg>
                        </button>
                        {selectedModel === model.id && hasModels && (
                          <div className="grid grid-cols-2 gap-2 pl-2 animate-in fade-in slide-in-from-top-2 duration-200">
                            {categoryModels.map((variation) => (
                              <button
                                key={variation.id}
                                onClick={() => handleModelChange(model.id, variation.id)}
                                className={`relative rounded-xl border-2 p-3 transition-all duration-200 group text-left ${
                                  selectedVariation === variation.id
                                    ? "border-sky-500 bg-gradient-to-br from-sky-500/25 to-cyan-500/15 shadow-md shadow-sky-500/20"
                                    : "border-slate-800 bg-slate-900/50 hover:border-slate-700 hover:bg-slate-900"
                                }`}
                              >
                                <div className="text-2xl mb-1.5 group-hover:scale-110 transition-transform">{variation.icon || model.icon}</div>
                                <p className="text-xs font-semibold text-slate-200 line-clamp-1">{variation.name}</p>
                                <p className="text-[10px] text-slate-500 line-clamp-1 mt-0.5">{variation.description}</p>
                                {selectedVariation === variation.id && (
                        ...
                        {selectedModel === model.id && !hasModels && (
                          <div className="pl-2 p-3 rounded-xl border border-slate-800 bg-slate-900/50 text-center">
                            <p className="text-xs text-slate-500">No models available yet</p>
                          </div>
                        )}
                      </div>
                    );
        ...
        {/* ── Canvas Area ── */}
        <div className="flex-1 relative bg-[radial-gradient(circle_at_top,_rgba(56,189,248,0.10),_transparent_55%),_radial-gradient(circle_at_bottom,_rgba(56,189,248,0.06),_transparent_55%)] pb-16 lg:pb-0">
          {/* Controls */}
          <div className="absolute top-4 right-4 z-10 flex flex-col gap-2">
            <label className="flex items-center gap-2 cursor-pointer bg-slate-900/90 backdrop-blur-md border border-slate-800/50 rounded-xl px-3 py-2 shadow-lg hover:bg-slate-900 transition-colors group">
              <input
                type="checkbox"
                checked={autoRotate}
              ...
              <svg className={`w-4 h-4 text-sky-400 ${autoRotate ? 'animate-spin' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {logoImages.length === 0 && (
            <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
              <div className="rounded-2xl bg-slate-950/90 backdrop-blur-sm px-8 py-6 border border-slate-800/50 text-center shadow-2xl max-w-sm mx-4">
                ...
                <p className="text-base font-bold text-slate-100 mb-2">Upload artwork to start</p>
                <p className="text-xs text-slate-400">Use the panel on the left to add an image and customize your mockup</p>
              </div>
            </div>
          )}
        </div>
      ...
      <div className="lg:hidden fixed bottom-0 inset-x-0 z-50 bg-slate-900/98 border-t border-slate-800/60 backdrop-blur-xl">
        <div className="flex">
          <button
            onClick={() => setActiveTab("edit")}
            className={`flex-1 flex flex-col items-center gap-1 py-3 transition-colors ${activeTab === "edit" ? "text-sky-400" : "text-slate-500 hover:text-slate-300"}`}
          >
          ...
          <button
            onClick={() => setActiveTab("model")}
            className={`flex-1 flex flex-col items-center gap-1 py-3 transition-colors ${activeTab === "model" ? "text-sky-400" : "text-slate-500 hover:text-slate-300"}`}
          >
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z" />
            </svg>
            <span className="text-[10px] font-semibold">Model</span>
          </button>
        </div>
      </div>

      {/* ── Mobile Slide-up Panel ── */}
      <div className="lg:hidden fixed bottom-14 inset-x-0 z-40 max-h-[55vh] overflow-y-auto bg-slate-900/98 border-t border-slate-800/60 backdrop-blur-xl rounded-t-2xl shadow-2xl custom-scrollbar">
        <div className="p-4">
          {activeTab === "edit" ? (
            <div className="space-y-5">
              <div>
                ...
                <ModelUVDataLoader selectedModel={selectedModel} selectedVariation={selectedVariation}>
                  {(uvData) => (
                    <UVEditor images={logoImages} uvPositions={uvPositions} uvScales={uvScales} onEditClick={() => setIsUVModalOpen(true)} baseColor={baseColor} uvData={uvData} />
                  )}
                </ModelUVDataLoader>
              </div>
              <div>
                ...
                <div className="flex items-center gap-3">
                  <input type="color" value={baseColor} onChange={(e) => setBaseColor(e.target.value)} className="h-12 w-12 rounded-lg border border-slate-700 bg-slate-900 cursor-pointer" />
                  <input type="text" value={baseColor} onChange={(e) => setBaseColor(e.target.value)} className="flex-1 rounded-md border border-slate-700 bg-slate-900 px-3 py-2 text-sm font-mono text-slate-100" />
                </div>
              </div>
            </div>
          ) : (
            <div className="space-y-3">
              <h2 className="text-sm font-semibold mb-3 text-slate-200">Select Model</h2>
              {MODEL_OPTIONS.map((model) => (
                <div key={model.id} className="space-y-2">
                  <button
                    onClick={() => { setSelectedModel(model.id); setSelectedVariation(null); }}
                    className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl border transition-all ${
                      selectedModel === model.id ? "border-sky-500 bg-sky-500/15 text-sky-100" : "border-slate-800 bg-slate-900/70 text-slate-300"
                    }`}
                  >
                    <span className="text-xl">{model.icon}</span>
                    <span className="font-medium text-sm flex-1 text-left">{model.label}</span>
                    <svg className={`w-4 h-4 transition-transform ${selectedModel === model.id ? "rotate-90 text-sky-400" : "text-slate-600"}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                    </svg>
                  </button>
                  {selectedModel === model.id && (
                    <div className="grid grid-cols-3 gap-2 pl-10">
                      {(getModelsByCategory(model.id) || []).map((variation) => (
                        <button
                          key={variation.id}
                          onClick={() => handleModelChange(model.id, variation.id)}
                          className={`rounded-xl border p-2 transition-all text-left ${
                            selectedVariation === variation.id ? "border-sky-500 bg-sky-500/20" : "border-slate-800 bg-slate-900/70"
                          }`}
                        >
