...
export const apiClient = {
  ...
  // ── Mockup / background upload ──────────────────────────────────────────

  async saveBackground(token, { url, publicId, name, category }) {
    return this.request('/api/mockups/save', {
      method: 'POST',
      headers: { Authorization: `Bearer ${token}` },
      body: JSON.stringify({ url, publicId, name, category }),
    });
  },

  async getUserBackgrounds(token) {
    return this.request('/api/mockups', {
      method: 'GET',
      headers: { Authorization: `Bearer ${token}` },
    });
  },

  async deleteBackground(token, id) {
    return this.request(`/api/mockups/${id}`, {
      method: 'DELETE',
      headers: { Authorization: `Bearer ${token}` },
    });
  },
  ...
  async saveProject(token, { id, name, templateId, bgUrl, designsJson, thumbnail }) {
    return this.request('/api/projects', {
      method: 'POST',
      headers: { Authorization: `Bearer ${token}` },
      body: JSON.stringify({ id, name, templateId, bgUrl, designsJson, thumbnail }),
    });
  },
